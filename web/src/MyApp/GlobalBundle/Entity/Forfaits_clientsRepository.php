...
class Forfaits_clientsRepository extends EntityRepository
{
        ...
        public function getAfficheListeForfaitValide()
	{
        ...
        /**
         * Retourne les forfaits de ce groupe d'hébergeemnts
         */
	...
	{
		$today = new \DateTime("now");
		$sql = 'SELECT p FROM MyAppGlobalBundle:Forfaits_clients p WHERE :now BETWEEN p.date_creation AND p.date_de_fin AND p.hebergement_id = :id AND p.actif = 1 AND p.forfait_client_id = 9  ';
		$query = $this->getEntityManager()->createQuery($sql);
		$query->setParameter('id', $id);
		$query->setParameter('now', $today);
		return $query->getResult();
	...
	{
		$today = new \DateTime("now");
		$sql = 'SELECT p FROM MyAppGlobalBundle:Forfaits_clients p JOIN p.hebergement_id q WHERE p.actif = 1 AND :now BETWEEN date_creation AND p.date_de_fin GROUP BY q.region_id ';
		$query = $this->getEntityManager()->createQuery($sql);
		$query->setParameter('now', $today);
		return $query->getResult();
	}
	...
	{
		$today = new \DateTime("now");
		$sql = 'SELECT p FROM MyAppGlobalBundle:Forfaits_clients p WHERE :now BETWEEN p.date_creation AND p.date_de_fin AND p.hebergement_id = :id AND p.actif = 1 AND p.forfait_client_id IN (7, 8)  ';
		$query = $this->getEntityManager()->createQuery($sql);
		$query->setParameter('id', $id);
		$query->setParameter('now', $today);
		return $query->getResult();
	...
	{
		$today = new \DateTime("now");
		$sql = 'SELECT p FROM MyAppGlobalBundle:Forfaits_clients p JOIN p.hebergement_id q WHERE p.actif = 1 AND :now BETWEEN p.date_creation AND p.date_de_fin AND p.derniere_minute = 1 AND q.province_id = :id GROUP BY q.region_id ';
		$query = $this->getEntityManager()->createQuery($sql);
		$query->setParameter('id', $id);
		$query->setParameter('now', $today);
		return $query->getResult();
	...
	public function getPaginationProvinceDerniereMinute($page, $limit, $id)
	{
		...
		$paginator = ($page - 1)* $limit;
		if(is_array($id) == 0){
			$sql = 'SELECT p FROM MyAppGlobalBundle:Forfaits_clients p JOIN p.hebergement_id q WHERE p.actif = 1 AND :now BETWEEN p.date_creation AND p.date_de_fin AND p.derniere_minute = 1 AND q.province_id = :id ORDER BY p.id DESC ';
		}else{    
			$sql = 'SELECT p FROM MyAppGlobalBundle:Forfaits_clients p JOIN p.hebergement_id q WHERE p.actif = 1 AND :now BETWEEN p.date_creation AND p.date_de_fin AND p.derniere_minute = 1 AND q.province_id IN (:id) ORDER BY p.id DESC ';
		}
		$query = $this->getEntityManager()->createQuery($sql);
		$query->setParameter('id', $id);
		$query->setParameter('now', $today);
        ...
        {
                $today = new \DateTime("now");		
                $sql = 'SELECT COUNT(p.id) AS nbPromotion FROM MyAppGlobalBundle:Forfaits_clients p JOIN p.hebergement_id q WHERE p.actif = 1 AND :now BETWEEN p.date_creation AND p.date_de_fin AND p.derniere_minute = 1 AND q.province_id IN (:id) ORDER BY p.id DESC ';
		$query = $this->getEntityManager()->createQuery($sql);
		$query->setParameter('id', $id);
		$query->setParameter('now', $today);	
	...
	{
		$today = new \DateTime("now");
		$sql = 'SELECT COUNT(p.id) AS numb FROM MyAppGlobalBundle:Forfaits_clients p JOIN p.hebergement_id q WHERE p.actif = 1 AND :now BETWEEN p.date_creation AND p.date_de_fin AND p.derniere_minute = 1 AND q.province_id = :id ';
		$query = $this->getEntityManager()->createQuery($sql);
		$query->setParameter('id', $id);
		$query->setParameter('now', $today);
		return $query->getResult();
	...
	public function getRechercheRegionDerniereMinute($page, $limit, $id)
	{
		$today = new \DateTime("now");
		$paginator = ($page - 1)* $limit;
		$sql = 'SELECT p FROM MyAppGlobalBundle:Forfaits_clients p JOIN p.hebergement_id q WHERE p.actif = 1 AND :now BETWEEN p.date_creation AND p.date_de_fin AND p.derniere_minute = 1 AND q.region_id = :id ORDER BY p.id DESC';
		$query = $this->getEntityManager()->createQuery($sql);
		$query->setParameter('id', $id);
		$query->setParameter('now', $today);
		$query->setFirstResult($paginator);
	...
	public function getRechercheVilleDerniereMinute($page, $limit, $id)
	{
		$today = new \DateTime("now");
		$paginator = ($page - 1)* $limit;
		$sql = 'SELECT p FROM MyAppGlobalBundle:Forfaits_clients p JOIN p.hebergement_id q WHERE p.actif = 1 AND :now BETWEEN p.date_creation AND p.date_de_fin AND p.derniere_minute = 1 AND q.ville_id = :id ORDER BY p.id DESC';
		$query = $this->getEntityManager()->createQuery($sql);
		$query->setParameter('id', $id);
		$query->setParameter('now', $today);
		$query->setFirstResult($paginator);
	...
	{
		$today = new \DateTime("now");
		$sql = 'SELECT COUNT(p.id) AS numb FROM MyAppGlobalBundle:Forfaits_clients p JOIN p.hebergement_id q WHERE p.actif = 1 AND :now BETWEEN p.date_creation AND p.date_de_fin AND p.derniere_minute = 1 AND q.region_id = :id ';
		$query = $this->getEntityManager()->createQuery($sql);
		$query->setParameter('id', $id);
		$query->setParameter('now', $today);
		return $query->getResult();
	...
	{
		$today = new \DateTime("now");
		$sql = 'SELECT p FROM MyAppGlobalBundle:Forfaits_clients p JOIN p.hebergement_id q WHERE p.actif = 1 AND :now BETWEEN p.date_creation AND p.date_de_fin AND p.derniere_minute = 1 AND q.region_id = :region AND p.forfait_client_id = :id ORDER BY p.id DESC';
		$query = $this->getEntityManager()->createQuery($sql);
		$query->setParameter('id', $id);
		$query->setParameter('region', $region);
		$query->setParameter('now', $today);
	...
	{
		$today = new \DateTime("now");
		$sql = 'SELECT p FROM MyAppGlobalBundle:Forfaits_clients p JOIN p.hebergement_id q WHERE p.actif = 1 AND :now BETWEEN p.date_creation AND p.date_de_fin AND p.derniere_minute = 1 AND q.ville_id = :ville AND p.forfait_client_id = :id ORDER BY p.id DESC';
		$query = $this->getEntityManager()->createQuery($sql);
		$query->setParameter('id', $id);
		$query->setParameter('ville', $ville);
		$query->setParameter('now', $today);
