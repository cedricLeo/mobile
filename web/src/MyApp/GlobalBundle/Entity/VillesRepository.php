...
class VillesRepository extends EntityRepository
{
	...
	{
		$sql = 'SELECT q FROM MyAppGlobalBundle:Villes q WHERE q.region_id = :id';
	...
	public function getListTown($page, $limit)
	{
		...
		$sql = 'SELECT p.id, p.nom_fr, p.nom_en FROM MyAppGlobalBundle:Villes p ORDER BY p.nom_fr ASC';
		$query = $this->getEntityManager()->createQuery($sql);
	...
	{
		$sql = 'SELECT p.id, p.nom_fr, p.nom_en, p.repertoire_fr, p.repertoire_en FROM MyAppGlobalBundle:Villes p WHERE p.nom_fr = :word OR p.nom_en = :word OR p.repertoire_fr =:word OR p.repertoire_en =:word';
		$query = $this->getEntityManager()->createQuery($sql);
	...
	{
		$sql = 'SELECT p.id, p.nom_fr, p.nom_en FROM MyAppGlobalBundle:Villes p WHERE p.nom_fr LIKE :letter ORDER BY p.nom_fr ASC';
		$query = $this->getEntityManager()->createQuery($sql);
	...
	{
		$sql = 'SELECT COUNT(p.id) AS nbville FROM MyAppGlobalBundle:Villes p';
		$query = $this->getEntityManager()->createQuery($sql);
		return $query->getResult();
	}
	...
	{
		$sql = 'SELECT p.id, p.nom_fr FROM MyAppGlobalBundle:Villes p';
		$query = $this->getEntityManager()->createQuery($sql);
		return $query->getResult();
	}
	...
	{
		$sql = 'SELECT p FROM MyAppGlobalBundle:Videos p WHERE p.ville =:id';
		$query = $this->getEntityManager()->createQuery($sql);
	...
	{
		$sql = 'SELECT MAX(p.id) AS lastid FROM MyAppGlobalBundle:Villes p ';
		$query = $this->getEntityManager()->createQuery($sql);
		return $query->getResult();
	}
	...
	{
		$sql = 'SELECT p FROM MyAppGlobalBundle:Villes p WHERE p.id =:id';
		$query = $this->getEntityManager()->createQuery($sql);
		$query->setParameter('id', $id);
		return $query->getResult();
	}
	...
	{
		$sql = 'SELECT p.id, p.nom_fr FROM MyAppGlobalBundle:Villes p WHERE p.nom_fr =:nomFR';
		$query = $this->getEntityManager()->createQuery($sql);
	...
	{	
		array_reverse($tablo);
		$sql = 'SELECT p FROM MyAppGlobalBundle:Villes p WHERE p.id IN (:tab)';
		$query = $this->getEntityManager()->createQuery($sql);
	...
	{
		$sql = 'SELECT p FROM MyAppGlobalBundle:Villes p WHERE p.zone_id = :zone_id ORDER BY p.nom_fr';
		$query = $this->getEntityManager()->createQuery($sql);
	...
	{
		$sql = 'SELECT p FROM MyAppGlobalBundle:Villes p WHERE p.nom_fr =:nom OR p.nom_en =:nom OR p.repertoire_fr =:nom OR p.repertoire_en =:nom';
		$query = $this->getEntityManager()->createQuery($sql);
	...
	{
		$sql = 'SELECT p FROM MyAppGlobalBundle:Villes p WHERE p.nom_fr = :word OR p.nom_en = :word OR p.repertoire_fr =:word OR p.repertoire_en =:word';
		$query = $this->getEntityManager()->createQuery($sql);
		$query->setParameter('word', $word);
		return $query->getResult();
	}
        ...
        /**
	 * Recherche l'id de la ville par son nom version mobile
	 */
	public function getSearchTownMobile($word)
	{
	...
	{
		$sql = 'SELECT p.reservit_id, p.nom_fr, p.nom_en FROM MyAppGlobalBundle:Villes p WHERE p.region_id = :id AND p.reservit_id is not null AND p.affiche = 1 AND p.reservit_id != 0';
		$query = $this->getEntityManager()->createQuery($sql);
		$query->setParameter('id', $id);
		return $query->getResult();
	}
	...
	public function getSelectListTown()
	{
        ...
        /**
         * Liste toutes les villes pour la réservation
         */
        public function getListAllVille()
        {
                $sql = 'SELECT p.reservit_id, p.nom_fr, p.nom_en FROM MyAppGlobalBundle:Villes p WHERE p.affiche = 1 AND p.reservit_id != 0';
		$query = $this->getEntityManager()->createQuery($sql);
		return $query->getResult();
        }
        
        /**
         * Liste toutes les villes qui sont dans le tableau de région
         */
        public function getListeVilleParTableauRegion($tab)
        {
                $sql = 'SELECT p.reservit_id, p.nom_fr, p.nom_en FROM MyAppGlobalBundle:Villes p WHERE p.region_id IN (:tab) AND p.affiche = 1 AND p.reservit_id != 0';
		$query = $this->getEntityManager()->createQuery($sql);
